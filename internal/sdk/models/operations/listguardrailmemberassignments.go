// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
// @generated-id: e6a7691c7449

package operations

import (
	"github.com/OpenRouterTeam/terraform-provider-openrouter/internal/sdk/internal/utils"
	"github.com/OpenRouterTeam/terraform-provider-openrouter/internal/sdk/models/shared"
	"net/http"
)

type ListGuardrailMemberAssignmentsRequest struct {
	// The unique identifier of the guardrail
	ID string `pathParam:"style=simple,explode=false,name=id"`
	// Number of records to skip for pagination
	Offset *int64 `default:"0" queryParam:"style=form,explode=true,name=offset"`
	// Maximum number of records to return (max 100)
	Limit *int64 `default:"50" queryParam:"style=form,explode=true,name=limit"`
}

func (l ListGuardrailMemberAssignmentsRequest) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(l, "", false)
}

func (l *ListGuardrailMemberAssignmentsRequest) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &l, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (l *ListGuardrailMemberAssignmentsRequest) GetID() string {
	if l == nil {
		return ""
	}
	return l.ID
}

func (l *ListGuardrailMemberAssignmentsRequest) GetOffset() *int64 {
	if l == nil {
		return nil
	}
	return l.Offset
}

func (l *ListGuardrailMemberAssignmentsRequest) GetLimit() *int64 {
	if l == nil {
		return nil
	}
	return l.Limit
}

type ListGuardrailMemberAssignmentsResponse struct {
	// HTTP response content type for this operation
	ContentType string
	// HTTP response status code for this operation
	StatusCode int
	// Raw HTTP response; suitable for custom response parsing
	RawResponse *http.Response
	// List of member assignments
	ListMemberAssignmentsResponse *shared.ListMemberAssignmentsResponse
	// Unauthorized - Authentication required or invalid credentials
	UnauthorizedResponse *shared.UnauthorizedResponse
	// Not Found - Resource does not exist
	NotFoundResponse *shared.NotFoundResponse
	// Internal Server Error - Unexpected server error
	InternalServerResponse *shared.InternalServerResponse

	Next func() (*ListGuardrailMemberAssignmentsResponse, error)
}

func (l ListGuardrailMemberAssignmentsResponse) MarshalJSON() ([]byte, error) {
	return utils.MarshalJSON(l, "", false)
}

func (l *ListGuardrailMemberAssignmentsResponse) UnmarshalJSON(data []byte) error {
	if err := utils.UnmarshalJSON(data, &l, "", false, nil); err != nil {
		return err
	}
	return nil
}

func (l *ListGuardrailMemberAssignmentsResponse) GetContentType() string {
	if l == nil {
		return ""
	}
	return l.ContentType
}

func (l *ListGuardrailMemberAssignmentsResponse) GetStatusCode() int {
	if l == nil {
		return 0
	}
	return l.StatusCode
}

func (l *ListGuardrailMemberAssignmentsResponse) GetRawResponse() *http.Response {
	if l == nil {
		return nil
	}
	return l.RawResponse
}

func (l *ListGuardrailMemberAssignmentsResponse) GetListMemberAssignmentsResponse() *shared.ListMemberAssignmentsResponse {
	if l == nil {
		return nil
	}
	return l.ListMemberAssignmentsResponse
}

func (l *ListGuardrailMemberAssignmentsResponse) GetUnauthorizedResponse() *shared.UnauthorizedResponse {
	if l == nil {
		return nil
	}
	return l.UnauthorizedResponse
}

func (l *ListGuardrailMemberAssignmentsResponse) GetNotFoundResponse() *shared.NotFoundResponse {
	if l == nil {
		return nil
	}
	return l.NotFoundResponse
}

func (l *ListGuardrailMemberAssignmentsResponse) GetInternalServerResponse() *shared.InternalServerResponse {
	if l == nil {
		return nil
	}
	return l.InternalServerResponse
}
